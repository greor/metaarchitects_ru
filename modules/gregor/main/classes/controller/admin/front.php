	...
	public function before()
	{
		...
		if ( Route::name( Request::current()->route() ) == 'modules' ) {
			$this->module_config = empty($this->module_config)
				? Helper_Module::code_by_controller(Request::current()->controller())
				: $this->module_config;
			
			$_pages = $this->get_module_pages($this->module_config);
			$this->module_page_id = (int) Request::current()->query('page');
		...
		$this->back_url = $this->request->current()->query('back_url');
		$this->is_cancel = (Request::$current->post('cancel') == 'cancel');
		$this->is_initial = $this->request->is_initial();
